...
import aiohttp
import os
import asyncio # WICHTIG: Muss importiert werden!
from datetime import datetime

class LogsArchiver(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        # V1 Client Key von https://www.warcraftlogs.com/profile
    ...
    async def check_logs(self): # Loop muss async sein
        """Prüft alle 15 Minuten auf neue Gilden-Logs."""
        if not self.wcl_api_key or not self.archive_channel_id:
            return
        await self.fetch_latest_logs()

    async def fetch_latest_logs(self):
        # Guild Name für URL formatieren (Leerzeichen zu %20)
        formatted_guild = self.guild_name.replace(" ", "%20")
        url = f"https://www.warcraftlogs.com:443/v1/reports/guild/{formatted_guild}/{self.realm}/{self.region}?api_key={self.wcl_api_key}"
