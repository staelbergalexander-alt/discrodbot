...
class SuperQuickModal(discord.ui.Modal, title='Neuer Gilden-Eintrag'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Macht aus %C3%AE wieder echte Zeichen wie î
        decoded_url = urllib.parse.unquote(self.rio_link.value)
        
        # Regex zum Extrahieren von Realm und Name aus der URL
        ...
        # Für die API-Anfrage müssen wir sie wieder sicher kodieren
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(api_url) as resp:
                if resp.status == 200:
                    ...
                    # Wir nehmen den Namen direkt von Raider.io für korrekte Groß/Kleinschreibung
        ...
        try:
            ...
            # 2. Robuste ID-Erkennung per Regex
            uid_match = re.search(r'(\d+)', msg.content)
            if not uid_match:
                return await interaction.followup.send("❌ Keine gültige Discord-ID/Erwähnung gefunden!", ephemeral=True)
            ...
            if member:
                ...
                # 3. Datenbank Speicherung (UTF-8 sicher)
                ...
                db_data[str(member.id)] = {"chars": [{"name": final_name, "realm": final_srv}]}
            ...
            await interaction.followup.send("❌ Zeit abgelaufen. Bitte erneut versuchen.", ephemeral=True)
